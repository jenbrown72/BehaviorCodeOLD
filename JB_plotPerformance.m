function [avgVelocity] = JB_plotPerformance(basicPropertiesToPlot,plotON)
...
for j = 1:length(numPoints);
    subplot(noSubPlots,1,1)
    plot(numPoints(j),basicPropertiesToPlot{j}.avgVelocity,'or','MarkerSize', 10,'MarkerFaceColor','r')
    %  plot(numPoints(j),basicPropertiesToPlot{j}.totalStepsPerMin,'or','MarkerSize', 10,'MarkerFaceColor','r')
    hold on
    avgVelocity(j,1) = basicPropertiesToPlot{j}.avgVelocity;
end
...
subplot(noSubPlots,1,2)
SessionTypes = {'S1auto' ; 'S1'; 'S2'; 'S6'; 'S8'; 'S10'; 'S12';'S0'};
...
for j = 1:length(numPoints);
    ...
    elseif strcmp('S12',basicPropertiesToPlot{j,1}.sessionType)
        colorCode(j,1) = 7;
    elseif strcmp('S0',basicPropertiesToPlot{j,1}.sessionType)
        colorCode(j,1) = 8;
    end
end
...
ColorCodeColors = [1.0;0.9;0.8;0.6;0.4;0.2;0.1;0];
...
%             X = ['Date ', basicPropertiesToPlot{jj,1}.date(1:11), ' Session', basicPropertiesToPlot{jj,1}.sessionType, ' Performance ', num2str( basicPropertiesToPlot{jj,1}.sessionperformance), ' dPrime ', num2str(basicPropertiesToPlot{jj,1}.dprime), ' #trials ',num2str(basicPropertiesToPlot{jj,1}.noTrials),' Duration ' ,num2str(basicPropertiesToPlot{jj,1}.sessionDuration)];
%             disp(X)
    ...
    plot(numPoints(jj),basicPropertiesToPlot{jj,1}.performanceCatch,'o','MarkerSize', 5, 'linewidth',2,'MarkerEdgeColor',[1,0,0], 'MarkerFaceColor',[1,0,0],'Color', [1,0,0]);
...
for jj = 1:length(numPoints);
    ...
    hold on
        plot(numPoints(jj),basicPropertiesToPlot{jj,1}.dPrimeCatch,'o','MarkerSize', 5, 'linewidth',2,'MarkerEdgeColor',[1,0,0], 'MarkerFaceColor',[1,0,0],'Color', [1,0,0]);
