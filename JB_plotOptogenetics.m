...
for h = 1:length(numPoints)
    ...
    if (basicPropertiesToPlot{h,1}.optogenetics)==1
        ...
        %           DATAstim.lickBias(tn,:) = ((basicPropertiesToPlot{h,1}.HIT +basicPropertiesToPlot{h,1}.FA) / (basicPropertiesToPlot{h,1}.HIT +basicPropertiesToPlot{h,1}.FA+basicPropertiesToPlot{h,1}.CR +basicPropertiesToPlot{h,1}.MISS));
        ...
        for gg=1:length(basicPropertiesToPlot{h,1}.angleLick)
            ...
            % meanLatency(:,2) = nanmean(basicPropertiesToPlot{h,1}.angleLickLatency{gg,2});
        ...
        set(gca,'XTick',linspace(L(1),L(2),NumTicks),'XTickLabel',labels,'XAxisLocation','top','ylim',[0 1000])
        view(-90,90)
...
if(stimulatedSession==1)
    ...
    if any(DATAstim.dPrime)
        ...
        for k=1:length(tempNames);
            ...
            plot(tempDATA','k-')
            hold on
            errorbar(meandPrime,SEMdPrime,'b-','LineWidth',3)
            %          if h==1
            %         sigstar(sigPairs,sigPairsPval,0,0.2)
            %     end
            set(gca,'XTick',[1:2]);
    ...
    plot(meanLatencies,'o-k','MarkerSize',3,'LineWidth',3)
    hold on
    errorbar(meanLatencies,stdLatencies,'o-k','MarkerSize',3,'LineWidth',1)
    ...
    labels = num2str(tempangles);
    set(gca,'XTick',linspace(L(1),L(2),NumTicks),'XTickLabel',labels,'XAxisLocation','top')
    view(-90,90)
